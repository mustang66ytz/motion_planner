...
log.basicConfig(format='%(asctime)s  %(levelname)-8s [%(name)s] %(module)s:%(funcName)s:%(lineno)d: %(message)s',
                level=log.INFO)
...
def params_from_file(config_files, overrides):
    config_string = ''
...
def write_params(params, config_file):
    ...
    config = pyhocon.ConfigFactory.from_dict(params)
